...
class VlRenderWorker(QThread):
    ...
    def request_stop(self) -> None:
        ...
        Safe to call from any thread (the extra ``QObject*`` argument
        from ``destroyed`` is ignored).
        """
        try:
            with self._condition:
                self._stopping = True
    ...
    def _on_destroyed(self, _obj: object = None) -> None:
        ...
        Accepts and ignores the ``destroyed(QObject*)`` argument and
        reaps with a bounded wait; on timeout the thread is left to quit
        itself on render completion.
        """
        try:
    ...
    def stop_bounded(self, timeout_ms: int) -> bool:
        ...
        if self.wait(timeout_ms):
            return True
        logger.warning(
            "VL renderer thread still running after %d ms; "
