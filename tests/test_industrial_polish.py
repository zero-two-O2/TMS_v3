...
def test_shelf_dimensions_reduced() -> None:
    ...
    assert config_window_module.PANEL_TAB_MIN_HEIGHT == 84
    assert config_window_module.PANEL_TAB_MAX_HEIGHT == 280
...
def test_explicit_widget_type_scales_in_stylesheet() -> None:
    ...
    assert "font-size: 8px;" in base
    assert "font-size: 10px;" in large  # round(8 * 1.3)
